...
export default function Help() {
  ...
  const handleSaveBody = async () => {
    if (!bodyModal.subheading.trim()) return toast("Subheading cannot be empty");
    try {
      if (bodyModal.id) {
        toast("Updating Body...");
        await editBody({ id: bodyModal.id, subheading: bodyModal.subheading, details: bodyModal.details });
        toast('Body updated');
      } else {
        toast("Creating Body...");
        await createBody({ heading: bodyModal.headingId, subheading: bodyModal.subheading, details: bodyModal.details });
        toast('Body created');
      }
      refetch();
      setBodyModal({ open: false, subheading: '', details: '', headingId: '', id: null });
    } catch {
  ...
  return (
    <div className="p-4">
      <div className="flex justify-between mb-4">
        ...
        <Button onClick={() => setHeadingModal({ open: true, title: '', id: null })}>
          <Plus className="w-4 h-4 mr-2" /> Add Heading
        </Button>
      </div>

      {data?.map((heading) => (
        <div key={heading.id} className="mb-6 border rounded p-4 bg-muted">
          <div className="flex justify-between items-center mb-2">
            <h3 className="text-lg font-semibold">{heading.title}</h3>
            <div className="flex gap-2">
              <Button variant="ghost" size="sm" onClick={() => setHeadingModal({ open: true, title: heading.title, id: heading.id })}>
                <Pencil className="w-4 h-4" />
              </Button>
              <Button variant="ghost" size="sm" onClick={() => setShowDeleteConfirm({ headingId: heading.id, bodyId: null })}>
                <Trash2 className="w-4 h-4 text-red-500" />
              </Button>
              <Button variant="ghost" size="sm" onClick={() => setBodyModal({ open: true, subheading: '', details: '', headingId: heading.id, id: null })}>
          ...
          {heading.bodies.map((body) => (
            <div key={body._id} className="mb-2 border rounded px-4 py-2 bg-white">
              <div className="flex justify-between items-center">
                <div
                  className="w-full cursor-pointer"
                  onClick={() => setExpandedBodyId(prev => prev === body._id ? null : body._id)}
                >
                  <p className="font-medium">{body.subheading}</p>
                  {expandedBodyId === body._id && <p className="mt-1 text-sm text-gray-700">{body.details}</p>}
                </div>
                <div className="flex gap-2 ml-2">
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => setBodyModal({ open: true, id: body._id, headingId: heading.id, subheading: body.subheading, details: body.details })}
                  >
                    <Pencil className="w-4 h-4" />
                  </Button>
                  <Button variant="ghost" size="sm" onClick={() => setShowDeleteConfirm({ headingId: null, bodyId: body._id })}>
                    <Trash2 className="w-4 h-4 text-red-500" />
                  </Button>
                </div>
              </div>
